...
UI:Section("Calibração do Auto Press")

UI:Button("Offset: +0.01s","Adianta o press em 0.01s. Útil se o clique estiver chegando tarde.",function()
    PRESS_OFFSET = math.min(0.20, PRESS_OFFSET + 0.01)
    UI:Notify("Calibração", ("Offset: %+.2fs"):format(PRESS_OFFSET), "info")
    print(("[STOP TIMER] Offset configurado: %+.2fs"):format(PRESS_OFFSET))
end)

UI:Button("Offset: -0.01s","Atrasa o press em 0.01s. Útil se estiver parando antes do alvo.",function()
    PRESS_OFFSET = math.max(-0.20, PRESS_OFFSET - 0.01)
    UI:Notify("Calibração", ("Offset: %+.2fs"):format(PRESS_OFFSET), "info")
    print(("[STOP TIMER] Offset configurado: %+.2fs"):format(PRESS_OFFSET))
end)

UI:Button("Resetar offset","Volta para 0.00s (modo exato).",function()
    PRESS_OFFSET = 0
    UI:Notify("Calibração", "Offset resetado para 0.00s", "info")
    print("[STOP TIMER] Offset resetado: 0.00s")
end)
...
print("[ZYRO HUB] Stop The Timer v2.3 CALIBRATION carregado")
-- ZYRO HUB • STOP THE TIMER v2
local Players = game:GetService("Players")
local UIS = game:GetService("UserInputService")
local VIM = game:GetService("VirtualInputManager")
